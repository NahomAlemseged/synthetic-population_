...
def main():
    ...
    # keep everything as string (required for GReaT)
    for col in features:
        train_subset[col] = train_subset[col].astype(str)

    # 🔥 REDUCED SAMPLE (major speed boost)
    train_sample = train_subset.sample(n=10000, random_state=42)

    print(f"Training GReaT model on {len(train_sample)} records...")

    # ⚡ lighter config (faster training)
    model = GReaT(
        llm='distilgpt2',   # you can also try: 'gpt2' (sometimes faster on GPU)
        batch_size=16,
        epochs=1
    )
    ...
    pool_size = 10000  # 🔥 reduced from 50k

    print(f"Generating {pool_size} synthetic records for mapping pool...")

    try:
        # ⚡ faster generation settings
        generated_pool = model.sample(
            n_samples=pool_size,
            k=10,               # reduced from 50
            max_length=128,     # huge speed improvement
            guided_sampling=True
        )
    except Exception as e:
        print(f"Sampling failed with error: {e}. Falling back to training data.")
        generated_pool = train_sample.copy()

    print("Creating MDC to ICD mapping...")

    # ⚡ limit list size per group (prevents memory explosion)
    icd_mapping = (
        generated_pool.groupby('APR_MDC')['PRINC_DIAG_CODE']
        .apply(lambda x: x.sample(min(len(x), 5)).tolist())
        .to_dict()
    )

    # ensure fast lookup
    icd_mapping = {str(k): v for k, v in icd_mapping.items()}

    def get_icd(mdc):
        vals = icd_mapping.get(str(mdc))
        return random.choice(vals) if vals else "UNKNOWN"

    print("Applying generated ICD codes...")

    # ⚡ faster than apply()
    synth_df['PRINC_DIAG_CODE'] = synth_df['APR_MDC'].astype(str).map(get_icd)
